...
typedef MultiSliderCallback(double lowerValue, double upperValue);

/// A Material Design range slider, extension of the original Flutter Slider.
///
/// Used to select a range of values using 2 thumbs.
///
/// A MultiSlider can be used to select from either a continuous or a discrete set of
...
/// [max] is 50.0 and [divisions] is 5, then the MultiSlider can take on the
/// discrete values 0.0, 10.0, 20.0, 30.0, 40.0, and 50.0.
///
/// The terms for the parts of a MultiSlider are:
///
...
///
/// The MultiSlider will be disabled if [onChanged] is null.
///
/// The MultiSlider widget itself does not maintain any state. Instead, when the state
/// of the MultiSlider changes, the widget calls the [onChanged] callback. Most
/// widgets that use a MultiSlider will listen for the [onChanged] callback and
/// rebuild the MultiSlider with a new set of values [lowerValue] and [upperValue]
/// to update the visual appearance of the MultiSlider.
///
/// To know when the values start to change, or when it is done
/// changing, set the optional callbacks [onChangeStart] and/or [onChangeEnd].
///
/// By default, a MultiSlider will be as wide as possible, centered vertically. When
...
/// a MultiSlider uses the [SliderThemeData] available from either a [SliderTheme]
...
///    the visual appearance of the MultiSlider.
  class MultiSlider extends StatefulWidget {
  /// Creates a material design MultiSlider.
  ///
  /// The MultiSlider itself does not maintain any state. Instead, when the state of
  /// the MultiSlider changes, the widget calls the [onChanged] callback. Most
  /// widgets that use a MultiSlider will listen for the [onChanged] callback and
  /// rebuild the MultiSlider with a new set of values ([lowerValue] and [upperValue])
  /// to update the visual appearance of the MultiSlider.
  ///
  /// * [lowerValue] determines the currently selected lower value for this MultiSlider.
  /// * [upperValue] determines the currently selected upper value for this MultiSlider.
  ...
  ///   for the MultiSlider.
  /// * [showValueIndicator] determines whether the MultiSlider should show a "value indicator" when
  ...
  final String thumbImageAsset;


  /// Callback to invoke when the user is changing the
  /// values.
  ///
  /// The MultiSlider passes both values [lowerValue] and [upperValue]
  /// to the callback but does not actually change state until the parent
  /// widget rebuilds the MultiSlider with the new values.
  ///
  /// If null, the MultiSlider will be displayed as disabled.
  ///
  ...
  ///    changing the value.
  final MultiSliderCallback onChanged;

  /// Callback to invoke when the user starts dragging
  ///
  /// This callback shouldn't be used to update the MultiSlider values ([lowerValue] or [upperValue])
  /// (use [onChanged] for that), but rather to be notified when the user has started
  /// selecting a new value by starting a drag.
  ///
  /// The values passed will be the last [lowerValue] and [upperValue] that the MultiSlider had before the
  /// change began.
  ///
  /// ## Sample code
  ///
  ...
  ///    complete.
  final MultiSliderCallback onChangeStart;
  ...
  ///    begins.
  final MultiSliderCallback onChangeEnd;
...
class _MultiSliderState extends State<MultiSlider>
  ...
  Widget build(BuildContext context) {
    return new _MultiSliderRenderObjectWidget(
      ...
      thumbImageAsset: widget.thumbImageAsset,
    );
  }
}
...
// ------------------------------------------------------
class _MultiSliderRenderObjectWidget extends LeafRenderObjectWidget {
  const _MultiSliderRenderObjectWidget({
    Key key,
    this.lowerValue,
    this.upperValue,
    this.divisions,
  ...
  RenderObject createRenderObject(BuildContext context) {
    return new _RenderMultiSlider(
      ...
      thumbImageAsset:thumbImageAsset,
    );
  }

  @override
  void updateRenderObject(
      BuildContext context, _RenderMultiSlider renderObject) {
    renderObject
      ..lowerValue = lowerValue
      ..upperValue = upperValue
      ..divisions = divisions
...
// ------------------------------------------------------
// Class that renders the MultiSlider as a pure drawing
// in a Canvas and allows the user to interact.
// ------------------------------------------------------
class _RenderMultiSlider extends RenderBox {
  skyui.Image thumbImage = null;
  _RenderMultiSlider({
    double lowerValue,
    double upperValue,
    int divisions,
    MultiSliderCallback onChanged,
    MultiSliderCallback onChangeStart,
    MultiSliderCallback onChangeEnd,
    SliderThemeData sliderTheme,
    @required this.state,
    bool showValueIndicator,
    int valueIndicatorMaxDecimals,
    double touchRadiusExpansionRatio,
    String thumbImageAsset,
  }) {
    // Initialization
    ...
    this.thumbImageAsset=thumbImageAsset;
  ...
  // -------------------------------------------------
  _MultiSliderState state;
  MultiSliderCallback _onChanged;
  MultiSliderCallback _onChangeStart;
  MultiSliderCallback _onChangeEnd;
  double _lowerValue;
  double _upperValue;
  String _thumbImageAsset;
  ...
  // _MultiSliderRenderObjectWidget.updateRenderObject
  // --------------------------------------------------
  ...
  set thumbImageAsset(String image) {
    _thumbImageAsset = image;
  }
  ...
  set onChanged(MultiSliderCallback value) {
  ...
  set onChangeStart(MultiSliderCallback value) {
    _onChangeStart = value;
  }

  set onChangeEnd(MultiSliderCallback value) {
    _onChangeEnd = value;
  }
  ...
  void _paintThumbs(PaintingContext context, Offset offset) {
    ...
    if(_thumbImageAsset!=null) {
      loadImage();
      if(thumbImage!=null) {
        context.canvas.drawImage(
            thumbImage, new Offset(_thumbLeftPosition-(thumbImage.width/2), _trackVerticalCenter-(thumbImage.height/2)), new Paint()..color = Colors.blue);
        context.canvas.drawImage(
            thumbImage, new Offset(_thumbRightPosition-(thumbImage.width/2), _trackVerticalCenter-(thumbImage.height/2)), new Paint()..color = Colors.blue);
      }
    }
    if(thumbImage==null){
  ...
  loadImage() async {
    if (thumbImage == null) {
      thumbImage = await ImageLoader.load(_thumbImageAsset);
      this.markNeedsPaint();
    }
  }
